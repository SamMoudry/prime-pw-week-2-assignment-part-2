...
//Then we check if code is greater than 250 if it is, but it is not, we would set variable secret equal to 'duper'
...
//We then check if isStudent is the same type as true and zip is less than 80000 if so, we would console.log 'You're a student on the West Coast'.
//We also check if isStudent is the same type as false or is age less than 30 if so, we would console.log 'What are your hobbies?'
//We then check if isStudent is the same type as true if so, which it is, we console.log 'Welcome to Prime'.
//Then as a catch all if none of the above are true we console.log 'How about the weather?'
